...
public class FurnaceFuelSlot : InventorySlot
{
    ...
    public override void EndDrag(InventorySlot selectedSlot)
    {
        ...
        {
            ItemSlotData item = furnaceInstance.GetFuelItem();
            if (selectedSlot is FurnaceInputSlot)
            {
                ...
                furnaceInstance.SetFuelItem(retrievedItem);

                furnaceInstance.TryRunSystem();
            }
            else if (selectedSlot is HandInventorySlot)
            {
                ItemSlotData retrievedItem = inventoryInstance.ItemToHand(inventoryType, item);
