...
public class FurnaceResultSlot : InventorySlot
{
    ...
    public override void EndDrag(InventorySlot selectedSlot)
    {
        ...
        {
            ItemSlotData item = furnaceInstance.GetOutputItem();
            if (selectedSlot is FurnaceInputSlot)
            {
                // Set positions to default
                rectTransform.anchoredPosition = new Vector2(0, 0);

                furnaceInstance.TrySetInputItem(item);
            }
            else if (selectedSlot is FurnaceFuelSlot)
            {
                if (furnaceInstance.IsItemFuel(item) && furnaceInstance.GetOutputItem() == null)
                {
                    furnaceInstance.SetFuelItem(item);
            ...
            else if (selectedSlot.GetType() == typeof(InventorySlot))
            {
                ...
                furnaceInstance.SetOutputItem(retrievedItem);
            }
